...
public class ConeDetection : MonoBehaviour
{
    ...
    [Tooltip("Determines how fast of an object it can see. Has a direct affect on performance.")]
    public float detectionSpeed = 0.08f; 
    Color color;
    ...
    IEnumerator getCollisionData()
    {
        while (this.enabled)
        {
            float rand = UnityEngine.Random.value * (detectionSpeed*2f);
            yield return new WaitForSeconds( (detectionSpeed/1.25f) + rand);
            objectsInView.Clear();
            FindCollision();
            if (obscructedView)
