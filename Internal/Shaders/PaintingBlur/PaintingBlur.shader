	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				uniform sampler2D _PaintingMap; //The render texture of THIS Camera/command buffer.
				...
				{
					fixed4 normalScreen = tex2D(_MainTex, i.uv);
					...
					//Exclude any part not in the buffer.
					fixed4 paintMap = tex2D(_PaintingMap, i.uv);
					//return paintMap;
					if(paintMap.r > 0.01)
						return image;
