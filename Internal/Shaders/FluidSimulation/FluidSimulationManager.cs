...
public class FluidSimulationManager : MonoBehaviour
{
    ...
    unsafe void BuildBVH()
    {
        ...
        */
        ...
        _fluidSimulationComputeShader.SetInt("_NumOfNodes", 5);
    ...
    void UpdateParticles()
    {
        if (_particleBuffer == null)
        {
            ...
            _MortonCodesBuffer = new ComputeBuffer(_MortonCodes.Length, sizeof(uint)*2);
            _MortonCodesTempBuffer = new ComputeBuffer(_MortonCodes.Length, sizeof(uint)*2);
        ...
        for (int i = 0; i < NumOfParticles; i++)
        {
            Debug.Log("Particle: " + _MortonCodes[i].particleIndex + "Particle IDs: " + _ParticleIDs[i] +  " Current Node: " + i + " Parent Node: " + _BVHNodes[i].parent +  " Left Child: " + _BVHNodes[i].leftChild  + " Right Child: " + _BVHNodes[i].rightChild +  " Nodes Contained " + _BVHNodes[i].primitiveOffset + "-" + (_BVHNodes[i].primitiveCount+ _BVHNodes[i].primitiveOffset) + " Morton Code: " + _MortonCodes[i].mortonCode.ToString("F7"));
        }
        //_fluidSimulationComputeShader.Dispatch(_PrefixSumKernelId, Mathf.CeilToInt(NumOfParticles / _prefixSumThreadSize.x), 1, 1);
