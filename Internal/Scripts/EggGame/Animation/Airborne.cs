...
public class Airborne : StateMachineBehaviour
{
    AgentPhysics agent;
    ...
    private void CheckAgentStates(AgentPhysics agent, Animator animator, AnimatorStateInfo stateInfo)
    {
        ...
        else if (agent.SubStates["airborne"].GetCurrentStateAsString() == "falling")
        {
            animationControllerSprites.SetAnimation("Falling");

        }
        else if (agent.SubStates["airborne"].GetCurrentStateAsString() == "rising")
        {
            animationControllerSprites.SetAnimation("Rising");
        }
    }
}
